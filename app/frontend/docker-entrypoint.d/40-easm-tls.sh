...
if [ "${EASM_TLS:-on}" = "off" ]; then
  rm -f "$CONF"
  echo "[easm-tls] EASM_TLS=off — HTTPS-Listener (443) deaktiviert"
  exit 0
fi

if [ ! -f "$TLS_DIR/easm.crt" ] || [ ! -f "$TLS_DIR/easm.key" ]; then
  SAN="${EASM_TLS_SAN:-DNS:localhost,DNS:easm.local,IP:127.0.0.1}"
  echo "[easm-tls] Erzeuge Self-Signed-Zertifikat (SAN: $SAN)"
  ...
  echo "[easm-tls] Bestehendes Zertifikat gefunden ($TLS_DIR/easm.crt)"
fi
...
if [ "${EASM_HSTS:-off}" = "on" ]; then
  ...
  echo "[easm-tls] HSTS aktiviert"
fi

echo "[easm-tls] HTTPS-Listener aktiv (Port 443)"
