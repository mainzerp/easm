...
> **NOTE:** This file contains instructions for the coding agent (Kilo/Orchestrator) and is **not part of the EASM application**. It does not define app behavior, runtime logic, or user-facing functionality.
>
...
YOU (Orchestrator): Post-Research Clarification (via `question` tool)
    ...
    - Example: "Should the new endpoint be REST or GraphQL?"
    - Example: "Do you want the UI as a modal or a separate page?"
...
YOU (Orchestrator): Spawn Planning subagent
    ...
    |
SUBAGENT #2: Planning
    - Prompt enforces: read, grep, glob, write (docs/SubAgent/ only) ONLY.
